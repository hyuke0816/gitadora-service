...
(function () {
  ...
  async function uploadSkillData(records, profileInfo, version) {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/skill-records`, {
        ...
        credentials: "include", // 쿠키 인증 포함 (세션 연동을 위해 필요)
  ...
  function loadSweetAlert2() {
    return new Promise((resolve, reject) => {
      ...
      script.onerror = () => reject(new Error("SweetAlert2 load failed"));
  ...
  async function main() {
    ...
    try {
      ...
      // 로딩 오버레이 메시지 업데이트
      updateLoadingMessage("프로필 정보 확인 중...");
