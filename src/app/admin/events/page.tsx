...
const formSchema = z.object({
  name: z.string().trim().min(1, "이벤트명을 입력해주세요"),
...
export default function Events() {
  ...
  const handleEdit = useCallback((event: Event) => {
    ...
    setFormValues({
      name: event.name,
      startedAt: format(new Date(event.startedAt), "yyyy-MM-dd"),
      endedAt: event.endedAt
        ? format(new Date(event.endedAt), "yyyy-MM-dd")
        : "",
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter((event) => {
      ...
      // 이벤트 타입으로 검색
      if (event.eventType.toLowerCase().includes(query)) {
        return true;
      }
      return false;
    });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateEvent({ id: editingId, ...validatedData });
      } else {
      ...
      setError(
        `이벤트 정보를 ${editingId ? "수정" : "등록"}하는데 실패했습니다: ${
          error.message
        }`
      );
    }
          ...
          <div className="shadow-xl rounded-xl p-8 mb-10 border border-gray-200/50 dark:border-gray-700/50 backdrop-blur-sm bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <option value="">이벤트 타입 선택</option>
                  <option value="이벤트">이벤트</option>
                  <option value="대회">대회</option>
                  <option value="프로모션">프로모션</option>
                  <option value="업데이트">업데이트</option>
                  <option value="곡 해금">곡 해금</option>
